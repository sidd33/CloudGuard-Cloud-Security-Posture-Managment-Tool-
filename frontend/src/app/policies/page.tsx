...
export default function PoliciesPage() {
  ...
  useEffect(() => {
    ...
    const cacheP = typeof window !== 'undefined' ? sessionStorage.getItem('policiesCache') : null;
    const cacheT = typeof window !== 'undefined' ? sessionStorage.getItem('templatesCache') : null;
    if (cacheP && cacheT) {
      try {
        setPolicies(JSON.parse(cacheP));
        setTemplates(JSON.parse(cacheT));
      } catch(e) {}
    }
    fetchData();
  }, []);
  ...
  const fetchData = async () => {
    try {
      ...
      if (typeof window !== 'undefined') {
        sessionStorage.setItem('policiesCache', JSON.stringify(polRes.data));
        sessionStorage.setItem('templatesCache', JSON.stringify(tempRes.data));
      }
